...
- **命名空间**:`<source>:<name>`(如 `local:io.read_file`、`remote:bot-1.search`)。`local:` 可省略保持 v0.2.0 兼容(AI 旧调用 `io.read_file` 仍有效)。
...
**任务清单**:
- [ ] `evermcp/security/auth.py`:API key 校验(hash 比对)+ WS 握手中间件。 **[M]**
- [ ] `evermcp/protocol/ws_channel.py`:WS 端点(移植 MCPPort 思路),握手 → 创建 `RemoteClientProvider`;心跳 30s;掉线移除能力。 **[L]**
- [ ] `evermcp/core/provider.py:RemoteClientProvider`:官方 `ClientSession`(over WS)桥接,`list_tools`/`call_tool` 经 WS 下发 JSON-RPC。 **[L]**
- [ ] `evermcp/storage.py`:加 `Client`、`ApiKey`、`CallLog` 表。 **[S]**
- [ ] `evermcp/connect/stdio_ws_bridge.py`:客户端适配器(启动 stdio server + 出站 WS 桥接);CLI `evermcp-connect --gateway ... --token K -- <cmd>`。 **[L]**
- [ ] `evermcp/protocol/rest_api.py`:`GET /api/clients`、API key CRUD、`GET /api/logs`(挂载到 web app)。 **[M]**
- [ ] 调用日志:`CallLog` 持久化 + UI 查询。 **[M]**
- [ ] 错误码:远程掉线/超时复用 `-32002/-32003`;新增 `remote_call_timeout_s`(默认 60s)。 **[S]**
- [ ] 测试:`tests/integration/test_ws_channel.py`(真实 stdio echo server)、`tests/e2e/test_remote_call.py`。 **[L]**
...
**总规模**: ~860行 / **总预估**: L(约 3-4 周,WS 桥接是难点)
